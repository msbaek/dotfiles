...
- [ ] **Step 1: 각 파일의 argument-hint 일괄 추출**
...
- [ ] **Step 2: 각 파일에 `## 실행 모델 (필수)` 섹션 삽입 (변형 B)**
...
- [ ] **Step 1: 각 파일 현재 상태 확인 (백그라운드 호출 코드가 본문에 있는지)**
...
- [ ] **Step 2: 각 파일에 `## 실행 모델 (필수)` 섹션 삽입 (변형 C)**
...
- [ ] **Step 1: 각 파일의 frontmatter `model:` 라인 제거**
...
- [ ] **Step 1: 각 파일 frontmatter 확인 (현재 model 값 + name)**
...
- [ ] **Step 2: `model:` 라인 제거 (있는 파일만) + 인터랙티브 주석 추가**
...
- [ ] **Step 1: 전체 변경 파일 일괄 검증 — 변형 A/B/C reference 존재**
...
- [ ] **Step 2: 인터랙티브 파일 검증 (commands 6 + skills 4 = 10 expected)**
...
- [ ] **Step 3: 인터랙티브 파일에 model 라인 부재 확인**
...
- [ ] **Step 5: Frontmatter YAML 무결성 (변경된 모든 파일)**
