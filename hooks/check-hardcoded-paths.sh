...
FILES_WITH_PATHS=$(git diff --cached --name-only | xargs grep -l "/Users/msbaek" 2>/dev/null | grep -v "SECURITY.md" | grep -v "check-hardcoded-paths.sh" | grep -v "^\.serena/" | grep -v "^\.codex/" | grep -v "^\.agents/" || true)
