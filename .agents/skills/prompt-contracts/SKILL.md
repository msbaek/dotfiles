...
Codex는 점쟁이가 아니라 **계약자(contractor)** — 명확한 계약 없이는 "화장실이 주방에 있는 집"을 받게 된다.
...
제약 없이는 Codex가 **기술 스택을 완전히 재발명**해 버린다.
...
명시적 포맷 지시 없이는 Codex는 **유지보수성이 아닌 속도에 최적화**한다.
...
프로젝트별 Constraints를 `AGENTS.md`에 문서화하여 **영구적 제약 계층(Permanent Constraint Layer)** 으로 활용:

```markdown
# AGENTS.md — Project Constraints (always active)
...
AGENTS.md 도입 전에는 Codex가 스택을 임의로 교체하는 일이 빈번했다.
AGENTS.md 도입 후에는 세션 간 일관성이 보장된다.
...
```
> Read AGENTS.md and confirm you understand the project constraints
> before doing anything.
```

이는 코드베이스의 **미란다 권리(Miranda rights)** — Codex가 제약을 되풀이하고,
