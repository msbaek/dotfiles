...
| Codex 세션 | `~/.Codex/history.jsonl` (세션 인덱스) |
...
#### SubAgent 2: Codex Sessions & Learning Analyzer (history.jsonl)

**Task 호출 파라미터:**
| 파라미터 | 값 |
|---------|-----|
| description | "Codex 세션 및 학습 분석" |
| subagent_type | "general-purpose" |
| model | "sonnet" |
...
```
당신은 Codex 세션 분석 및 학습 내용 추출 전문가입니다. 코드를 작성하지 말고 분석만 수행하세요.

## 작업
{TARGET_DATE} 날짜의 Codex 세션을 ~/.Codex/history.jsonl 파일에서 파싱하여:
1. 수행한 작업을 프로젝트별로 요약
2. 학습 관련 내용을 추출하여 분류
...
with open(os.path.expanduser('~/.Codex/history.jsonl')) as f:
    lines = f.readlines()
...
## 에러 처리
- history.jsonl 파일 없음 → "history.jsonl 파일을 찾을 수 없음 — Codex 세션 분석 건너뜀" 반환
...
- history.jsonl 파일 없음: SubAgent 2가 "history.jsonl 파일을 찾을 수 없음 — Codex 세션 분석 건너뜀" 반환, 나머지 서브 에이전트 정상 동작
...
- `weekly-Codex-analytics`: 주간 종합 분석
...
| MCP 서버 | 용도 | 등록 명령 |
|----------|------|----------|
| Things MCP | SubAgent 4에서 Things 활동 분석 | `Codex mcp add-json -s user things '{"command":"uvx","args":["things-mcp"]}'` |

> **참고**: Things MCP 서버가 등록되어 있지 않아도 스킬은 정상 동작합니다. 해당 서브 에이전트만 "건너뜀" 처리됩니다. SubAgent 2는 `~/.Codex/history.jsonl` 파일을 직접 파싱하므로 별도 MCP 서버가 필요 없습니다.
