...
```bash
[ -f ~/.Codex/state/vis-backlink/.disabled ] && echo "DISABLED" || echo "OK"
```
...
```bash
mkdir -p ~/.Codex/state/vis-backlink/active
mkdir -p ~/.Codex/state/vis-backlink/history
```

동시성 체크:
```bash
ls ~/.Codex/state/vis-backlink/active/*.json 2>/dev/null
```
...
```bash
JOB_ID="<job_id>"
STATE_FILE=~/.Codex/state/vis-backlink/active/${JOB_ID}.json
TMP_FILE=$(mktemp)
cat > "$TMP_FILE" << STATEEOF
{
  ...
  "targets": [],
  "log_path": "~/.Codex/logs/vis-backlink-$(date +%Y%m%d).log"
}
STATEEOF
...
- `prompt`: 아래 내용 포함
  ...
  - state JSON path: `~/.Codex/state/vis-backlink/active/<job_id>.json`
  - vault_root 경로

dispatch 완료 후 메인 Codex **즉시 해제** (4초 이내).

**history/ 정리:**
```bash
ls -t ~/.Codex/state/vis-backlink/history/ | tail -n +31 | xargs -I{} rm ~/.Codex/state/vis-backlink/history/{}
```
