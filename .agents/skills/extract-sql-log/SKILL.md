...
**~/.Codex/templates/delegation.md 변형 B 적용** — 단, `subagent_type`은 `general-purpose` 대신 **`sql-log-extractor`** 사용 (전용 sub-agent).
...
python3 ~/.Codex/skills/extract-sql-log/scripts/extract_sql.py /tmp/sql_log_input.txt

# 전체 DML 포함 (SELECT 포함)
python3 ~/.Codex/skills/extract-sql-log/scripts/extract_sql.py --all /tmp/sql_log_input.txt

# JSON 출력 (프로그래밍 활용)
python3 ~/.Codex/skills/extract-sql-log/scripts/extract_sql.py --json /tmp/sql_log_input.txt

# 파일로 저장
python3 ~/.Codex/skills/extract-sql-log/scripts/extract_sql.py --all -o /tmp/sql_analysis.md /tmp/sql_log_input.txt
```

### Step 3: 분석 보강 (Codex 판단 영역)

Python 파서가 기계적 추출을 하고, Codex는 다음을 추가 분석한다:
