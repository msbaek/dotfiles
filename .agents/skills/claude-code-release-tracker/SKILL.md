...
description: |
  ...
  매일 morning-auto.sh에서 자동 호출되거나, 수동으로 /Codex-release-tracker 실행.
...
**~/.Codex/templates/delegation.md 변형 A 적용** — 단, `subagent_type`은 `general-purpose` 대신 **`Codex-release-fetcher`** 사용 (전용 sub-agent).
...
X(@claudecodelog)에서 Codex 릴리스 노트를 수집하여 한글로 번역/구조화하고
...
1. Read 도구로 `~/DocumentsLocal/msbaek_vault/scripts/Codex-release-tracker-state.json` 읽기
...
2. 대상 파일 경로 결정: `~/DocumentsLocal/msbaek_vault/001-INBOX/Codex-releases-YYYY-MM.md`
