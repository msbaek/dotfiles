---
name: html-review
description: Codex의 응답(브레인스토밍·분석·계획·결정사항 등)을 인터랙티브 HTML 리뷰 페이지로 변환. 각 항목에 👍채택/🤔의문/🔁수정/❌거절 버튼, 코멘트, LocalStorage 자동저장, Markdown·프롬프트 export 기능 포함. ~/Desktop에 저장 후 브라우저로 자동 열림.
license: MIT
metadata:
  version: "1.0"
  author: msbaek
...
**~/.Codex/templates/delegation.md 변형 A 적용** — 단, `subagent_type`은 `general-purpose` 대신 **`html-review-builder`** 사용 (전용 sub-agent).
...
Codex의 응답·분석·계획을 light-themed 단일 HTML 파일로 변환하는 skill.

- **각 항목 = 결정 1건 + 객관식 옵션 리스트** (정확히 하나에 ✅ 권장 마크)
- **버튼 의미**: 👍 권장안 채택 / 🤔 다른 옵션 (코멘트에 라벨) / 🔁 옵션 자체 수정 / ❌ 전부 거절
- **자동저장**: LocalStorage (날짜-슬러그 key)
- **Export**: Markdown / "Copy as next prompt" (피드백 → Codex 재요청)
- **저장 위치**: `~/Desktop/Codex-review-{날짜}-{슬러그}.html`
...
`~/.Codex/skills/html-review/assets/` 에 CSS 템플릿 저장. agent가 Read 도구로 참조.
