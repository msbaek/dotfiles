---
name: "source-command-skills-curate"
description: "audit 결과 기반 대화형 skill 정리 (unused/overlap 결정 기록)"
---

# source-command-skills-curate

Use this skill when the user asks to run the migrated source command `skills-curate`.
...
```bash
~/.Codex/bin/skills-curate.py
```

재개:
```bash
~/.Codex/bin/skills-curate.py --resume
```
...
```bash
# archive 예시
mkdir -p ~/.Codex/skills-archive
mv ~/.Codex/skills/<name> ~/.Codex/skills-archive/
