---
name: brunch-writer
description: 브런치 블로그 글 작성을 도와주는 Skill. 사용자가 초안 파일(.md)을 @멘션하면서 "브런치", "글 작성", "블로그 글" 등을 언급하면 활성화. vault-intelligence 시스템으로 관련 자료를 검색하고, 구조 제안, 스타일 체크 등을 수행. 단, Codex가 직접 글을 쓰지 않고 사용자의 글쓰기를 보조하는 역할.
...
> **Codex는 정원사, 사용자는 나무**
> Codex가 글을 쓰는 것이 아니라, 사용자의 경험과 생각이라는 **씨앗**을 받아서 **자료 수집**, **구조화 제안**, **다듬기**로 나무가 자라도록 돕는 역할
...
> 사용자가 Codex에게 초안 작성을 요청한 경우, 아래 패턴을 사전에 적용하면 수정 횟수를 줄일 수 있다.
