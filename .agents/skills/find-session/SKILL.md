...
name: find-session
description: |
  자연어로 이전 Codex 세션을 검색하고 요약하는 스킬. agf 키워드 검색과 qmd 의미 검색을 병행하여
...
이전 Codex 세션을 자연어 설명으로 검색하여 찾고, 상세 정보와 요약을 제공하는 스킬.
...
Agent({
  ...
  name: "agf-searcher",
  prompt: `아래 조건으로 Codex 세션을 검색하고 결과를 보고해주세요.
...
## 실행할 명령어
${DATE가 특정 날짜인 경우}
  python3 ~/.Codex/skills/agf/list.py {YYYY-MM-DD}
${키워드가 있는 경우}
  python3 ~/.Codex/skills/agf/search.py "{KEYWORD}"
${결과가 3건 미만이면 deep 검색 추가}
  python3 ~/.Codex/skills/agf/search.py --deep "{KEYWORD}"
...
Agent({
  ...
  name: "qmd-searcher",
  prompt: `아래 쿼리로 Codex 세션을 의미 검색하고 결과를 보고해주세요.
코드를 작성하지 말고 검색만 수행하세요.
