...
name: agf
description: |
  Codex 세션 탐색 및 분석. agf 데이터 소스(history.jsonl)를 활용한 세션 리스트 조회, 검색 및 상세 분석.
...
**~/.Codex/templates/delegation.md 변형 B 적용** — 단, `subagent_type`은 `general-purpose` 대신 **`agf-session-explorer`** 사용 (전용 sub-agent).
...
agf(AI Agent Session Finder)의 데이터 소스인 `~/.Codex/history.jsonl`과 세션 JSONL 파일을 활용하여 Codex 세션을 프로그래밍 방식으로 탐색·분석하는 skill.
...
- `history.jsonl` 파일 없음 → "~/.Codex/history.jsonl 파일을 찾을 수 없습니다" 출력
