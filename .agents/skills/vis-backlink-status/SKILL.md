...
description: Use when user asks about background vis-backlink (reverse Related Notes update) progress. Reads ~/.Codex/state/vis-backlink/ and reports active/recent/failed jobs with progress bars. Trigger on "vis-backlink 상태", "역방향 진행", "backlink 어디까지", "/vis-backlink-status".
---
...
**~/.Codex/templates/delegation.md 변형 B 적용** — 단, `subagent_type`은 `general-purpose` 대신 **`vis-backlink-status-reader`** 사용 (전용 sub-agent).
...
0. Bash: `[ -f ~/.Codex/state/vis-backlink/.disabled ] && echo OFF || echo ON` → 활성화 상태.
1. Bash: `ls ~/.Codex/state/vis-backlink/active/*.json 2>/dev/null` → 활성 목록.
2. Bash: `ls -t ~/.Codex/state/vis-backlink/history/*.json 2>/dev/null | head -5` → 최근 history.
3. 각 JSON 파일을 Read.
4. 집계 후 아래 포맷으로 출력:
...
- 파일: `~/.Codex/state/vis-backlink/active/*.json`, `~/.Codex/state/vis-backlink/history/*.json`
...
- Hook: `~/.Codex/AGENTS.md` `<when-creating-obsidian-document>` backward 블록
