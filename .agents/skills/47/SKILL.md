...
This is for the **chat app** (Codex.ai, Mac, iOS), not the API. The user is going to paste a single message into chat. There is no system prompt, no `effort` parameter, no tool config to tune. The prompt itself has to do all the work.
...
**Case B — the user only described a class of task** ("I want a prompt to triage my emails", "help me prompt Codex to review my code", "give me a prompt for writing LinkedIn posts about my launches"). Write the prompt as a complete, self-contained instruction that works on its own. End the instruction by either:
- Asking Codex to ask the user for the specific inputs it needs ("Before drafting, ask me to share the product name, audience, and a link."), or
...
### Tell Codex what to do, not what to avoid
...
### Give Codex a role when it sharpens behavior
...
For analysis or Q&A over long inputs, instruct Codex to first pull relevant quotes into `<quotes>` tags, then answer based on those quotes. This dramatically reduces drift and hallucination.

### Be literal about scope
Opus 4.7 doesn't silently generalize. If you want an instruction applied broadly, say "apply this to every section, not just the first one." If you want Codex to take action rather than suggest, use imperative verbs ("Edit the function to..." not "Could you suggest improvements to..."). Suggestion-flavored phrasing produces suggestions.
...
The prompt ends by inviting the user to paste content in the next turn. No `[paste emails here]` block. The user sends this as-is and Codex responds with "Ready when you are — paste the batch."
...
Instead of a `<product_details>` block with `[paste details here]`, the prompt explicitly tells Codex to gather the details by asking. Self-contained, ready to send.

### Example 3 — Case B (technical task, no code given)

**User input:**
> I need Codex to review my python code for bugs
...
**The user gives you a Codex system prompt or API-style prompt with parameters.** Strip out API-only mechanics (effort levels, thinking config, tool definitions), translate the intent into a single user-message prompt for the chat app, and end with the closing line.

**The user wants the prompt to ask Codex to do many small things.** Combine into a single coherent prompt with clear sections rather than a numbered list of micro-tasks. Opus 4.7 handles long, well-structured asks well.
...
**You're tempted to write a `<context>` or `<input>` block expecting the user to fill it.** Don't. That's Rule 1. Either bake the actual content in (Case A) or tell Codex to ask the user for it (Case B).
