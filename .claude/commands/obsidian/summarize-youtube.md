...
if [[ "$CLEANED_ARGUMENTS" == *"youtube.com/watch?v="* ]] || [[ "$CLEANED_ARGUMENTS" == *"youtu.be/"* ]]; then
    ...
    # download-youtube-transcript 스크립트로 JSON 형식 데이터 추출 (언어 옵션 적용)
    if [ "$LANG_OPTION" = "kr" ]; then
        echo "한글 트랜스크립트로 다운로드 시도"
        YOUTUBE_DATA=$(~/bin/download-youtube-transcript -f json -l kr "$CLEANED_ARGUMENTS" 2>/dev/null || ~/bin/download-youtube-transcript -f json -l en "$CLEANED_ARGUMENTS")
    else
        echo "영어 트랜스크립트로 다운로드 시도"
        YOUTUBE_DATA=$(~/bin/download-youtube-transcript -f json -l en "$CLEANED_ARGUMENTS" 2>/dev/null || ~/bin/download-youtube-transcript -f json -l kr "$CLEANED_ARGUMENTS")
    fi
...
5. **관련 문서 추가 (Related Notes)**

   문서 생성이 완료되면, `vis search`를 사용하여 관련 문서를 찾고 Related Notes 섹션을 추가한다.

   1. **관련 문서 검색**: 생성된 문서의 제목과 핵심 키워드로 `vis search` 실행
      ```bash
      vis search "문서 핵심 키워드" --search-method hybrid --rerank --top-k 10
      ```
   2. **후보 필터링**: 검색 결과에서 다음을 제외
      - 자기 자신 (방금 생성한 문서)
      - daily notes (`notes/dailies/`)
      - 유사도가 낮은 문서
   3. **Related Notes 섹션 추가**: 필터링된 상위 3-5개 문서를 문서 하단에 추가
      ```markdown
      ## Related Notes
      - [[문서명]] - 맥락 설명
      ```
   4. **피드백 요청**:
      - **백그라운드 모드**: 작업 완료 알림 시 추가된 Related Notes 목록을 함께 보여주고, 사용자가 리뷰/수정할 수 있도록 안내
      - **동기 모드**: 후보 목록을 보여주고 사용자 승인 후 적용

6. **Progress 파일 업데이트 (백그라운드 모드 시)**
...
```json
{
  ...
  "related_notes_added": ["문서1.md", "문서2.md"],
  "error": null
}
```
