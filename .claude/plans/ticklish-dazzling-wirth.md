...
| 파일 | 변경 내용 |
|------|----------|
| `run_scrubbing.sh` | `--from-date=YYYY-MM-DD` 옵션 추가, 템플릿 처리 함수 |
| `02_scrub_hmmall_core.sql` | 플레이스홀더 추가 |
| `03_scrub_hmmall_extra.sql` | 플레이스홀더 추가 |
| `04_scrub_capybara.sql` | 플레이스홀더 추가 |
| `05_verify.sql` | 증분 검증 쿼리 추가 |
| `README.md` | 새 옵션 및 시나리오 문서화 |
...
1. **run_scrubbing.sh 옵션 파싱 확장** - `--from-date` 옵션 추가
2. **process_sql_template() 함수 구현** - sed 치환 로직
3. **02_scrub_hmmall_core.sql 플레이스홀더 추가**
4. **03_scrub_hmmall_extra.sql 플레이스홀더 추가**
5. **04_scrub_capybara.sql 플레이스홀더 추가**
6. **05_verify.sql 증분 검증 추가**
7. **README.md 문서 업데이트**
...
미확인 테이블들의 날짜 컬럼 조회:
```sql
DESCRIBE hmmall.USER;
DESCRIBE hmmall.FAN_EVENT_USER;
-- 등등
```

→ 스키마에 REG_DT/CREATED_AT 등이 있으면 해당 테이블도 증분 처리 가능
